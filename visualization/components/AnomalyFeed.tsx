...
const eventTypeConfig: Record<string, { label: string; icon: typeof AlertTriangle }> = {
  zscore_spike: { label: "Z-Score Spike", icon: AlertTriangle },
  whale_trade: { label: "Whale Trade", icon: Wallet },
  whale_directional_bias: { label: "Directional Bias", icon: TrendingUp },
  benford_violation: { label: "Benford Violation", icon: BarChart3 },
};
      ...
      <ScrollArea className="flex-1">
        <div className="flex flex-col divide-y divide-border">
          {anomalies.map((anomaly) => {
            const typeConf = eventTypeConfig[anomaly.eventType] ?? { label: anomaly.eventType, icon: AlertTriangle };
